    ...
    <v-layout justify-center row wrap>
      <v-flex lg5 xs12 md8 sm8>
        <v-card
          ...
          data-aos="fade-up"
          v-for="(ico,index) in upComing"
          :key="index"
        >
          <v-layout row wrap justify-space-between>
            <v-flex xs7 lg7 md6 sm7>
              <v-card-title>
                <div class="scroll">
                  ...
                  <div
                    class="pt-1"
                    v-if="ico.ico_start!=''&& ico.ico_start!=null"
                  >End date:{{ico.ico_start.slice(0,10)}}</div>
                </div>
              </v-card-title>
            </v-flex>
  ...
  asyncComputed: {
    async filterData() {
      return this.$store.state.ICODetails.filter(ico => {
        return ico.status.match("Upcoming");
      });
    },
    async upComing() {
...
<style>
  ...
  //     });
  //     return data.data.icoData.results.filter(ico => {
  //       console.log(ico);
  //       return ico.status.match("Upcoming");
  //     });
  //   }
